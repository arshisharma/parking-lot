...
func TestCommand_Run(t *testing.T) {
	...
	outPut := []struct {
		...
		{nil, "Allocated slot number: 4"},
		{custerr.ErrParkingFull, "Sorry, parking lot is full"},
		{nil, "KA-01-HH-1234 ,KA-01-HH-9999 ,KA-01-P-333"},
		{nil, "1 ,2 ,4"},
		{nil, "6"},
		{custerr.ErrNotFound, "Not found"},
	}
	commandMgr := NewManager()
	for idx, command := range commandStrings {
		out, err := commandMgr.Run(command)
		if command == "status" {
		...
		if out != outPut[idx].out && command != "status" {
